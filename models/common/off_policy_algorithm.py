...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    :param safe_mult: If use multiplicative value function
    """
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            # unscaled_action in [low, high]

        # Rescale the action from [low, high] to [-1, 1]
        if isinstance(self.action_space, gym.spaces.Box):
            scaled_action = self.policy.scale_action(unscaled_action)  # in [-1, 1] or [0, 1] for beta
            ...
            action = self.policy.unscale_action(scaled_action)  # from [-1,1] or [0, 1] to [low, high]
        else:
    ...
    def _dump_logs(self) -> None:
        ...
        if len(self.ep_info_buffer) > 0 and len(self.ep_info_buffer[0]) > 0:
            ...
            self.logger.record("important/train_rew", safe_mean([ep_info["r"] for ep_info in self.ep_info_buffer]))
    ...
    def custom_log(self):
        ...
        if 'use_beta_dist' in self.policy_kwargs.keys() and self.policy_kwargs['use_beta_dist']:
            self.logger.record("rollout/alpha", np.mean(self.actor.alpha_logging))
            self.logger.record("rollout/beta", np.mean(self.actor.beta_logging))
        elif not self.use_sde:
            self.logger.record("rollout/mean", np.mean(self.actor.mean_logging))
            self.logger.record("rollout/std", np.mean(self.actor.std_logging))
    ...
    def collect_rollouts(
        ...
        # Reset distribution log
        if self.__class__.__name__ == "SAC":
            self.actor.std_logging = []
            self.actor.mean_logging = []
            self.actor.alpha_logging = []
            self.actor.beta_logging = []
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            while not done:
                ...
                obs_tensor = self.policy.obs_to_tensor(self._last_obs)
                self.log_to_env(action, obs_tensor)
    ...
    def log_to_env(self, action, observation):
        with th.no_grad():
            if self.__class__.__name__ == 'DQN':
                q = self.q_net(observation).reshape(-1).cpu().numpy()
                q_action = q[action]
                c = self.col_net(observation).reshape(-1).cpu().numpy()
                c_action = c[action]
            elif self.__class__.__name__ == 'SAC':
                if self.env.action_space.shape[0] == 2:  # = Lunar Lander
                    # Nop, fire left engine, main engine, right engine
                    debug_a = np.array([[-1, 0], [0, -1], [1, 0], [0, 1]])
                else:
                    raise NotImplementedError("Debug Action for this action space not implemented")
                q_action = compute_critic_values(self.critic, observation, action, minimum=True)
                q = [compute_critic_values(self.critic, observation, a, minimum=True) for a in debug_a]
                c_action = compute_critic_values(self.col_net, observation, action, minimum=False)
                c = [compute_critic_values(self.col_net, observation, a, minimum=False) for a in debug_a]
            else:
                raise NotImplementedError("Class of type: " + self.__class__.__name__ + " is not implemented")
